  ...
  return (
    <div className="flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-3xl space-y-8">
        ...
        <div className="flex w-full flex-wrap gap-2">
          {CHAT_TAB_MESSAGE.map((tab, index) => (
            <Button
              ...
              className={"w-27.5 cursor-pointer justify-start"}
            >
              {tab.icon}
        ...
        <div className="min-h-60 w-full space-y-3">
          {CHAT_TAB_MESSAGE[activeTab].messages.map((msg, index) => (
            <div key={index}>
              <button
                onClick={() => onMessageSelect(message)}
                className="text-muted-foreground hover:text-primary w-full cursor-pointer py-2 text-left text-sm transition-colors duration-300 ease-in-out"
              >
                {msg}
              </button>
